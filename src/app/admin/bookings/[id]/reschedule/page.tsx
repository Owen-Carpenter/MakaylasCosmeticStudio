...
export default function AdminRescheduleAppointmentPage() {
  const router = useRouter();
  const params = useParams();
  const appointmentId = params.id as string;

  const [appointment, setAppointment] = useState<Appointment | null>(null);
  const [customerDetails, setCustomerDetails] = useState<{name?: string, email?: string}>({});
    ...
    if (appointmentId) {
      const fetchAppointmentDetails = async () => {
        ...
        try {
          ...
          // Also fetch the booking to get customer details (not included in Appointment interface)
          const bookingResponse = await fetch(`/api/bookings/${appointmentId}`);
          ...
          if (apptData) {
            ...
            if (bookingData.success && bookingData.booking) {
              setCustomerDetails({
                name: bookingData.booking.customer_name,
                email: bookingData.booking.customer_email
              });
            }
          } else {
    ...
    const available = timeSlotsForDate.filter(timeSlot => {
      ...
      if (overlappingBooking) {
        ...
        if (isOriginalDateAndTime) {
          timeDisplay[timeSlot] = { 
            available: true, 
            reason: 'Current appointment time'
          };
        } else {
  ...
  const handleReschedule = async () => {
    ...
    try {
      // Get the booking data to access user_id
      ...
      const success = await updateAppointmentDateTime(
        appointment.id,
        selectedDate,
        selectedTime,
        data.booking.user_id // Use the user_id from the booking data
      );

      if (success) {
        toast.success('Appointment rescheduled successfully');
        // Redirect to admin dashboard instead of booking details
        ...
        <div className="max-w-2xl mx-auto">
          <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-lg">
            <CardHeader className="bg-gradient-to-r from-primary/10 to-indigo-600/10 border-b">
              <CardTitle className="text-2xl text-center">Reschedule Customer Appointment</CardTitle>
              ...
              <div className="space-y-2">
                  ...
                  <>
                    ...
                    <div className="mt-2 text-xs text-gray-600">
                      ...
                      <div className="flex items-center mb-1">
                        ...
                        <span>Current appointment time</span>
                      </div>
                ...
                {selectedDate && selectedTime && (
                  <div className="text-xs text-gray-600 mt-2">
                    ...
                    <p>Appointment will end at approximately {
                      (() => {
