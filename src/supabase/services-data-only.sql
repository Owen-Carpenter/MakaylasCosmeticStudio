-- =====================================================
-- MAKAYLA'S COSMETIC STUDIO - SERVICES DATA ONLY
-- =====================================================
-- This script ONLY inserts service and variant data
-- Run this after migration scripts are complete
-- =====================================================

-- Clear existing data first (optional - comment out if you want to keep existing data)
-- DELETE FROM service_variants;
-- DELETE FROM services WHERE category IN ('lashes', 'facials', 'waxing');
...
-- Wait a moment for services to be inserted
SELECT pg_sleep(1);
...
ON CONFLICT (service_id, variant_name) DO NOTHING;
